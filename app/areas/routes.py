...
def create_area():
    ...
    exists = Area.query.filter_by(company_id=current_company_id(), name=name, deleted_at=None).first()
    if exists:
        flash("Ya existe un área con ese nombre.", "danger")
        return redirect(url_for("areas.index"))

    area = Area(company_id=current_company_id(), name=name, description=description, created_by=current_user.id)
    db.session.add(area)
    db.session.flush()
...
def create_task(area_id):
    area = Area.query.filter_by(id=area_id, company_id=current_company_id(), deleted_at=None).first_or_404()
    name = request.form.get("name", "").strip()
    description = request.form.get("description", "").strip() or None
    if not name:
    ...
    if exists:
        flash("Ya existe una tarea con ese nombre en el área.", "danger")
        return redirect(url_for("areas.index"))
...
def delete_area(area_id):
    ...
    db.session.commit()
    flash("Área eliminada lógicamente junto con sus tareas.", "success")
    return redirect(url_for("areas.index"))
...
def delete_task(task_id):
    task = (
        ...
        .filter(Task.id == task_id, Area.company_id == current_company_id(), Task.deleted_at.is_(None))
        .first_or_404()
    )
    task.soft_delete(current_user.id)
    task.active = False
    ...
    db.session.commit()
    flash("Tarea eliminada lógicamente.", "success")
    return redirect(url_for("areas.index"))
...
def tasks_json(area_id):
    tasks = (
        Task.query.join(Area)
        .filter(Area.company_id == current_company_id(), Area.id == area_id, Task.active.is_(True), Task.deleted_at.is_(None))
        .order_by(Task.name)
        .all()
    )
