...
def index():
    ...
    employees = Employee.query.filter_by(company_id=current_user.company_id, active=True, deleted_at=None).order_by(Employee.last_name).all()
    if current_user.role == "Employee" and not current_user.is_company_owner:
        employees = [current_user.employee] if current_user.employee else []

    areas = Area.query.filter_by(company_id=current_user.company_id, active=True, deleted_at=None).order_by(Area.name).all()
    records_query = TimeRecord.query.filter_by(company_id=current_user.company_id, deleted_at=None)
    if current_user.role == "Employee" and not current_user.is_company_owner:
...
def finish(record_id):
    record = TimeRecord.query.filter_by(id=record_id, company_id=current_user.company_id, deleted_at=None).first_or_404()
    if current_user.role == "Employee" and not current_user.is_company_owner and record.employee_id != current_user.employee_id:
